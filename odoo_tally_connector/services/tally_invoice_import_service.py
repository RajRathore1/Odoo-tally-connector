"""
Sales Voucher import service - Tally -> Odoo direction.

Orchestrates fetching Sales vouchers from Tally (within a date range) and
creating matching Odoo account.move (customer invoice) records.

Matching strategy (stable identity only - no name-based fallback, unlike
product/partner import): Tally's REMOTEID/GUID is the only identifier a
voucher carries, and it is opaque (no human-meaningful "name" to fall back
on the way a product or ledger name would). So:
1. account.move.tally_guid == the voucher's REMOTEID/GUID -> already
   imported (or originally pushed from Odoo and echoed back) -> skip.
2. Else -> create a new draft invoice.

Every dependency (customer, product, account, tax) must already be linked
to Tally (via prior sync/import) before a voucher references it - missing
mappings are reported as a per-voucher error, never guessed or silently
skipped. Imported invoices are left in draft so an accountant reviews them
before posting; this service never auto-posts.
"""
...
class TallyInvoiceImportService:
    """Imports Tally Sales vouchers into Odoo as draft customer invoices."""

    def __init__(self, env):
        self.env = env

    def import_invoices(self, connection, date_from, date_to):
        """
        Fetch Sales vouchers from Tally within a date range and create
        matching draft Odoo invoices.
        ...
        result = client.fetch_sales_vouchers(
        ...
        if not result["success"]:
            return {
                ...
                "skipped": [],
                "errors": [],
                "message": result.get("message", "Failed to fetch Sales vouchers from Tally"),
            }
        ...
        for voucher in result["vouchers"]:
            ...
            try:
                vals = self._build_invoice_vals(voucher, company)
                move = Move.create(vals)
                created.append(move.name)
                _logger.info(
                    f"Invoice created from Tally import: {move.name}",
                    extra={"move_id": move.id, "connection_id": connection.id, "tally_guid": guid},
                )
            except Exception as e:
                errors.append({"name": label, "error": str(e)})
                _logger.warning(
                    f"Error importing Tally Sales voucher '{label}': {str(e)}",
    ...
    def _build_invoice_vals(self, voucher, company):
        partner = self._resolve_partner(voucher.get("party_ledger_name"))
        ledger_entries = voucher.get("ledger_entries") or []
        inventory_entries = voucher.get("inventory_entries") or []

        if inventory_entries:
            # Item Invoice voucher: each stock item is its own line, its
            # revenue account comes from its own nested accounting
            # allocation, and every top-level (non-party) ledger entry is a
            # tax applied to every line - see build_sales_voucher_upsert_request
            # for why the structures are shaped this way.
            ...
            # Plain accounting voucher (Tally's "Accounting Voucher" view -
            # no stock items, the ledgers are posted directly): every
            # top-level (non-party) ledger entry is either a tax or a
            # revenue/expense account - one line per revenue/expense entry,
            # no product.
    ...
    def _resolve_partner(self, ledger_name):
        ...
        if not partners:
            raise TallyMappingError(
                ...
                f"sync or import this contact first."
            )
        if len(partners) > 1:
            raise TallyMappingError(
    ...
    def _build_plain_ledger_lines(self, ledger_entries):
        """
        Build invoice lines for a plain accounting voucher (no stock items).

        Every top-level ledger entry other than the party is classified by
        trying a tax mapping first, then an account mapping - whichever is
        configured. An entry matching neither is an explicit error (never
        guessed). Ambiguous requires unlinking. All resolved taxes are
        applied to every revenue/expense line, mirroring the Item Invoice
        path's tax handling.
        """
        tax_ids = []
        revenue_entries = []
