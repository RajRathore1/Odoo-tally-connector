"""
Purchase Voucher import service - Tally -> Odoo direction.

Orchestrates fetching Purchase vouchers from Tally (within a date range) and
creating matching Odoo account.move (vendor bill) records. Mirrors
tally_invoice_import_service.py's design exactly, with move_type="in_invoice"
and vendor resolution instead of customer resolution - see that module's
docstring for the matching strategy and rationale (GUID-only matching, no
name fallback; every dependency must already be linked to Tally; imported
bills are left in draft; plain accounting vouchers with no stock items are
supported as ledger-only lines).
"""
...
class TallyBillImportService:
    """Imports Tally Purchase vouchers into Odoo as draft vendor bills."""

    def __init__(self, env):
        self.env = env

    def import_bills(self, connection, date_from, date_to):
        """
        Fetch Purchase vouchers from Tally within a date range and create
        matching draft Odoo vendor bills.
        ...
        result = client.fetch_purchase_vouchers(
        ...
        if not result["success"]:
            return {
                ...
                "skipped": [],
                "errors": [],
                "message": result.get("message", "Failed to fetch Purchase vouchers from Tally"),
            }
        ...
        for voucher in result["vouchers"]:
            ...
            try:
                vals = self._build_bill_vals(voucher, company)
                move = Move.create(vals)
                created.append(move.name)
                _logger.info(
                    f"Bill created from Tally import: {move.name}",
                    extra={"move_id": move.id, "connection_id": connection.id, "tally_guid": guid},
                )
            except Exception as e:
                errors.append({"name": label, "error": str(e)})
                _logger.warning(
                    f"Error importing Tally Purchase voucher '{label}': {str(e)}",
    ...
    def _build_bill_vals(self, voucher, company):
    ...
    def _build_plain_ledger_lines(self, ledger_entries):
        """Same plain-accounting-voucher handling as TallyInvoiceImportService."""
        tax_ids = []
        revenue_entries = []
        ...
        if not revenue_entries:
            raise TallyMappingError(
                "Plain accounting voucher has no expense/revenue ledger entry to build an "
    ...
    def _resolve_partner(self, ledger_name):
        ...
        if not partners:
            raise TallyMappingError(
                ...
                f"sync or import this vendor first."
            )
        if len(partners) > 1:
            raise TallyMappingError(
